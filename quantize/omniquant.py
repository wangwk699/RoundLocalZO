...
def omniquant(
    ...
    for i in range(len(layers)):
                ...
                if isinstance(module, QuantLinear):
                    for key in pairs.keys():
                        if key in name:
                            ...
                            # ldx:add
                            # weight = module.orig_weight.abs().max(dim=0)[0].clamp(min=1e-5)
        ...
        # ANN ---> QANN其实是有两个过程，过程一：模型的层直接进行替换 过程二：加载参数，一种是直接加载微调后的好的量化参数，另一种是喂一点数据进行微调训练（我们不用这种方式）
        # ldx_add:如果有resume，我们就已经有了一个微调好的量化模型，我们就不需要进行epoch对量化过程中的scale/zero进行微调训练了。这时候记得把epoch设置为0                 
        if args.resume:
    ...
    # ldx:add:可以通过state_dict()获取所有参数
    # final_state_dict = model.state_dict()
    # if not os.path.exists(args.output_dir):
    #     os.makedirs(args.output_dir)
    # torch.save(final_state_dict, os.path.join(args.output_dir, "final_model.pth"))
    return model
