...
@triton.jit
def _aqlm_dequantization(
    codes_ptr,
    codebooks_ptr,
    out_ptr, # pointer to dequantized weights
    in_features: tl.constexpr,
    out_features: tl.constexpr,
    in_features_next_power_of_2: tl.constexpr,
    out_features_next_power_of_2: tl.constexpr,
    ...
    # sum along the dimension of num_codebooks
    weights_i = tl.sum(weights_i, axis=1)
    # ^-- [num_input_groups_next_power_of_2, out_group_size, in_group_size]

    out_group_ix = tl.arange(0, out_group_size)[:, None, None]
    num_in_group_ix = tl.arange(0, num_input_groups_next_power_of_2)[None, :, None]
    in_group_ix = tl.arange(0, in_group_size)[None, None, :]

    out_ptrs = out_ptr  \
               + pid * out_group_size * in_features \
               +  out_group_ix * num_input_groups_next_power_of_2 * in_group_size \
               + num_in_group_ix * in_group_size \
               + in_group_ix
    # ^-- [num_output_group, in_feature_next_power_of_2] ?

    # store the reconstructed weights
    tl.store(out_ptrs,
             weights_i.permute(1, 0, 2), # [out_group_size, num_input_groups_next_power_of_2, in_group_size]
             mask=codes_i_mask_1d[None, :, None])
...
def aqlm_gemm_stupid(
    ...
    _aqlm_dequantization[grid](
        codes_i16,
        codebooks,
        W,
        in_features,
        out_features,
        next_power_of_2(in_features),
        next_power_of_2(out_features),
        num_codebooks,
        codebook_size,
        out_group_size,
        in_group_size,
