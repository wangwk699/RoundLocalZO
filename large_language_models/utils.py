...
def encode_prompt(task, template, train_samples, eval_sample, tokenizer, max_length, sfc=False, icl_sfc=False, generation=False, generation_with_gold=False, max_new_tokens=None):
    ...
    if hasattr(tokenizer, 'add_bos_token'): # QZO Added: Avoid bugs when using PreTrainedTokenizerFast (for Llama-3)
...
def _load_state_dict_from_pretrained_dir(ckpt_dir: str):
    ...
    raise FileNotFoundError(
        f"Cannot find model.safetensors or pytorch_model.bin in {ckpt_dir}. "
        f"If you saved sharded weights, you need to load shards via transformers checkpoint loader."
    )

def _add_new_module(name, root, new_module):
    # 复刻你 omniquant 里的 add_new_module
    levels = name.split(".")
    if len(levels) == 1:
    ...
    mod_ = root
    for l in levels[:-1]:
        mod_ = mod_[int(l)] if l.isdigit() else getattr(mod_, l)
    setattr(mod_, levels[-1], new_module)

def _replace_mixtral_linears(layer, weight_quant_params, act_quant_params):
    # mixtral 在你 omniquant 里：只替换 Linear->QuantLinear，且跳过 gate
    for name, module in list(layer.named_modules()):
        if isinstance(module, torch.nn.Linear) and ("gate" not in name):
            ql = QuantLinear(module, weight_quant_params, act_quant_params)
            _add_new_module(name, layer, ql)

def rebuild_omniquant_structure(model, omni_net: str, weight_quant_params, act_quant_params, args_like):
    net = omni_net.lower()

    if "llama" in net or "mixtral" in net:
        layers = model.model.layers
        if "mixtral" in net:
            for i in range(len(layers)):
                _replace_mixtral_linears(layers[i], weight_quant_params, act_quant_params)
        else:
            for i in range(len(layers)):
                layers[i] = QuantLlamaDecoderLayer(model.config, layers[i], args_like)

    elif "opt" in net:
        layers = model.model.decoder.layers
        for i in range(len(layers)):
            layers[i] = QuantOPTDecoderLayer(model.config, layers[i], args_like)

    elif "falcon" in net:
        layers = model.transformer.h
        for i in range(len(layers)):
            layers[i] = QuantFalconDecoderLayer(model.config, layers[i], args_like)

    else:
        raise ValueError(f"Unsupported omni_net: {omni_net}")

    return model

def load_omniquant_model(args, free_in_GB: int):
    """
    返回：model, tokenizer
    """
    assert args.omni_ckpt_dir and os.path.isdir(args.omni_ckpt_dir), "omni_ckpt_dir is invalid"
    assert args.omni_base_model, "omni_base_model is required"
    assert args.omni_net, "omni_net is required"

    # 让 tokenizer/config 用量化目录的（通常 tokenizer 相同，但以 save_dir 为准更稳）
    config = AutoConfig.from_pretrained(args.omni_ckpt_dir)
    tokenizer = AutoTokenizer.from_pretrained(args.omni_ckpt_dir, use_fast=False)

    # 先加载 base fp 结构（不要直接 load ckpt_dir，因为 ckpt_dir 里是量化后的 state_dict）
    torch_dtype = torch.float16 if args.load_float16 else (torch.bfloat16 if args.load_bfloat16 else torch.float32)

    model = AutoModelForCausalLM.from_pretrained(
        args.omni_base_model,
        config=config,
        device_map="auto" if (not args.no_auto_device) else None,
        torch_dtype=torch_dtype,
        max_memory={i: f'{free_in_GB-5}GB' for i in range(torch.cuda.device_count())} if (not args.no_auto_device) else None,
        load_in_8bit=args.load_int8,
    )

    # 组装你 omniquant 所需的量化参数（字段名按你 QuantLinear/QuantXXX 的实现习惯）
    class _ArgsLike: pass
    qargs = _ArgsLike()

    # 这些字段会被 QuantXXX layer 用到（你 omniquant 里传的 args）
    qargs.net = args.omni_net
    qargs.wbits = args.omni_wbits
    qargs.abits = args.omni_abits
    qargs.group_size = args.omni_group_size
    qargs.symmetric = args.omni_symmetric
    qargs.disable_zero_point = args.omni_disable_zero_point
    qargs.w_dynamic_method = args.omni_w_dynamic_method
    qargs.a_dynamic_method = args.omni_a_dynamic_method
    qargs.let = args.omni_let
    qargs.lwc = args.omni_lwc
    qargs.real_quant = args.omni_real_quant

    qargs.weight_quant_params = {
        "n_bits": qargs.wbits,
        "per_channel_axes": [0],
        "symmetric": qargs.symmetric,
        "dynamic_method": qargs.w_dynamic_method,
        "group_size": qargs.group_size,
        "lwc": qargs.lwc,
        "disable_zero_point": qargs.disable_zero_point,
    }
    qargs.act_quant_params = {
        "n_bits": qargs.abits,
        "per_channel_axes": [],
        "symmetric": False,
        "dynamic_method": qargs.a_dynamic_method,
    }

    # 关键：重建量化结构
    model = rebuild_omniquant_structure(model, args.omni_net, qargs.weight_quant_params, qargs.act_quant_params, qargs)

    # 再加载量化权重
    sd = _load_state_dict_from_pretrained_dir(args.omni_ckpt_dir)

    # 你保存前删了一些临时字段，strict=True 很可能会 missing，所以这里用 strict=False
    missing, unexpected = model.load_state_dict(sd, strict=False)
    if len(missing) > 0:
        logger.warning(f"[OmniQuant] Missing keys: {len(missing)} (show 20) {missing[:20]}")
    if len(unexpected) > 0:
        logger.warning(f"[OmniQuant] Unexpected keys: {len(unexpected)} (show 20) {unexpected[:20]}")
