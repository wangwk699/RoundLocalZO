...
from utils import *
from trainer import ZOTrainer, QZOTrainer, QAZOTrainer
...
#     quant_method: str = ''   # supported quantization methods: gptq, aqlm
#     should_save: bool = True # save model weights or not; need to be False during memory profiling
#     clip_zo_grad: bool = False     # whether to activate Directional Derivative Clipping
#     train_unquantized: bool = False # if True, train unquantized parameters as well (LayerNorm, embedding layer)
#     # clear_activations: bool = False # if True, clear activations after a forward pass
...
class OurArguments(TrainingArguments):
    # dataset and sampling strategy
    task_name: str = "CB"  # 根据你的任务，可以替换为实际任务名 (如 CB, Copa, ReCoRD, 等)[SST2,BoolQ，RTE,CB]
    ...
    train_as_classification: bool = True  # 根据任务需求，CB, Copa 等任务设置为 False
...
def parse_args():
    # 手动传递 TrainingArguments 所需的必需参数
    args = OurArguments(
        output_dir="./log/cb-2.7")
    print(args)
    return args
...
def main():
    ...
    task = get_task(train_args.task_name)
    train_sets = task.sample_train_sets(num_train=train_args.num_train, num_dev=train_args.num_dev, num_eval=train_args.num_eval, num_train_sets=train_args.num_train_sets, seed=train_args.train_set_seed)
    ...
    framework = Framework(train_args, task, lm.model, lm.tokenizer)
    if train_args.train_set_seed is not None or train_args.num_train_sets is not None:
        # Eval samples share one (or multiple) training set(s)
        for train_set_id, train_samples in enumerate(train_sets):
            train_set_seed = train_set_id if train_args.train_set_seed is None else train_args.train_set_seed

            # Sample eval samples
            if train_args.num_eval is not None:
                eval_samples = task.sample_subset(data_split="valid", seed=train_set_seed, num=train_args.num_eval)
            else:
                eval_samples = task.valid_samples

            if train_args.trainer != "none":
                if train_args.num_dev is not None:
                    # Dev samples
                    dev_samples = train_samples[-train_args.num_dev:] 
                    train_samples = train_samples[:-train_args.num_dev]
                else:
                    dev_samples = None
                ...
                assert train_args.num_dev is None
            ...
            if not train_args.no_eval:
                ...
                if train_args.local_rank <= 0:
                    write_metrics_to_file(metrics, "result/" + result_file_tag(train_args) + f"-trainset{train_set_id}.json" if train_args.result_file is None else train_args.result_file)

    else:
        # For each eval sample, there is a training set. no training is allowed
        # This is for in-context learning (ICL)
        assert train_args.trainer == "none"
        if train_args.num_eval is not None:
            eval_samples = task.sample_subset(data_split="valid", seed=0, num=train_args.num_eval)
        else:
            eval_samples = task.valid_samples
        ...
        logger.info(metrics)
        if train_args.local_rank <= 0:
            write_metrics_to_file(metrics, "result/" + result_file_tag(train_args) + "-onetrainpereval.json" if train_args.result_file is None else train_args.result_file)    
