...
# --- 设置 Resume 路径 ---
if [ "$WBITS" -eq 2 ]; then
    RESUME="./pre_quantized_models/$MODEL-w2a16.pth"
elif [ "$WBITS" -eq 3 ]; then
    RESUME="./pre_quantized_models/$MODEL-w3a16g.pth"
...
# 构建完整路径
SAVE_DIR="./log7/$MODEL-w${WBITS}a${ABITS}/$METHOD/$TASK/MAX_LENGTH-$MAX_LENGTH-STEPS-$STEPS-IR-$IR$DIR_SUFFIX"

# --- 执行训练 ---
CUDA_VISIBLE_DEVICES=5,6 python train_main.py \
