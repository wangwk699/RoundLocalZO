...
# QZO Added: import QZO custom trainers
from trainer import ZOTrainer, QZOTrainer
import random
...
class OurArguments(TrainingArguments):
    ...
    #chents_add
    omni_ckpt_dir: str = ""      # 量化后 save_pretrained 的目录（save_dir）
    omni_base_model: str = ""    # 原始 FP 模型（HF id 或本地路径）
    omni_net: str = "" 
...
class Framework:
    ...
    def load_model(self):
        ...
        with count_time("Loading model with FP%d" % (16 if self.args.load_float16 else 32)):
            ...
            if self.args.untie_emb:
                ...
                # ZOQuant Added: load gptq-quantized model with GPTQModel library 
                if self.args.trainer == 'qzo' and self.args.quant_method == 'omniquant':
                    model, tokenizer = load_omniquant_model(self.args, free_in_GB)
                    # tokenizer 已经从 omni_ckpt_dir load 了，所以这里直接 return
                    return model, tokenizer
                elif self.args.trainer == 'qzo' and self.args.quant_method == 'gptq':
                    # For GPTQ
                    from gptqmodel import GPTQModel
                    model = GPTQModel.load(self.args.model_name, device_map='auto')
                
                else:
                    # For AQLM, MeZO
                    # AQLM models can be directly loaded with HuggingFace transformers
    ...
    def train(self, train_samples, eval_samples):
        ...
        # QZO Added: set QZOTrainer
            ...
            assert self.args.quant_method in ['gptq', 'aqlm'] # supported methods
            trainer = QZOTrainer(
                model=self.model, 
                args=self.args,
                train_dataset=train_dataset, 
...
def train_entry(cli_args=None):
    """
    让训练逻辑可以被 main.py 调用
    cli_args: list[str] 或 None
    """
    ...
    if cli_args is None:
        ...
        args = parser.parse_args_into_dataclasses(cli_args)[0]
    ...
    framework = Framework(args, task)
    # 如果你原 main() 里还有 evaluate / write_metrics，也照搬过来
    framework.train(train_sets)
