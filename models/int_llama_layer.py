...
class QuantLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        value_states = (self.v_proj(hidden_states)
                        .view(bsz, q_len, self.num_key_value_heads, self.head_dim)
                        .transpose(1, 2))
        # print(f"[DEBUG] Type of past_key_values: {type(past_key_values)}")
        # if past_key_values is not None:
        #     print(f"[DEBUG] past_key_values length: {len(past_key_values)}")
        #     if len(past_key_values) > 0:
        #         print(f"[DEBUG] Type of past_key_values[0]: {type(past_key_values[0])}")
                # print(f"[DEBUG] Attributes of past_key_values[0]: {dir(past_key_values[0])}")
        ...
        if past_key_values is not None:
            try:
                # 1. 优先检查是否为新版 Transformers 的 DynamicCache 或类似结构
                if hasattr(past_key_values, 'get_seq_length'):
                    past_length = past_key_values.get_seq_length()
                    kv_seq_len += past_length
                # 2. 检查是否有直接可用的 key 张量
                elif hasattr(past_key_values, 'key') and past_key_values.key is not None:
                    # 确保访问的是张量的序列长度维度
                    kv_seq_len += past_key_values.key.shape[-2]
                # 3. 处理可能的多层缓存结构（例如，past_key_values 是一个包含各层缓存的列表或元组）
                elif isinstance(past_key_values, (list, tuple)) and len(past_key_values) > 0:
                    # 获取第一个元素，它可能代表第一层的缓存
                    first_layer_cache = past_key_values[0]
                    # 3.1 如果第一层缓存是 DynamicCache 或类似对象
                    if hasattr(first_layer_cache, 'get_seq_length'):
                        past_length = first_layer_cache.get_seq_length()
                        kv_seq_len += past_length
                    # 3.2 如果第一层缓存是一个 (key, value) 元组
                    elif isinstance(first_layer_cache, (tuple, list)) and len(first_layer_cache) >= 2:
                        past_key_tensor = first_layer_cache[0]  # 假定第一个元素是 key 张量
                        if isinstance(past_key_tensor, torch.Tensor):
                            kv_seq_len += past_key_tensor.shape[-2]
                    # 3.3 其他未预料的结构，尝试直接获取第一个元素的形状
                    elif hasattr(first_layer_cache, 'shape'):
                        kv_seq_len += first_layer_cache.shape[-2]
                    else:
                        # 如果无法识别结构，记录警告，但避免报错，先不增加长度
                        print(
                            f"Warning: Unrecognized past_key_values[0] structure: {type(first_layer_cache)}. kv_seq_len may be incorrect.")
                else:
                    # 最后的保底尝试，直接使用 past_key_values 的第一个元素
                    try:
                        if hasattr(past_key_values[0], 'shape'):
                            kv_seq_len += past_key_values[0].shape[-2]
                    except (IndexError, TypeError):
                        # 如果所有方法都失败，记录错误但不中断程序，依赖于后续的形状检查来暴露问题
                        print(
                            f"Error: Could not determine sequence length from past_key_values of type {type(past_key_values)}.")
            except Exception as e:
                # 捕获计算过程中的任何意外错误
                print(
                    f"An error occurred while calculating kv_seq_len: {e}. Proceeding with current length {kv_seq_len}.")
        """
        ...
        # print(f"[DEBUG] Expected attn weights shape: ({bsz}, {self.num_heads}, {q_len}, {kv_seq_len})")
        # print(f"[DEBUG] Actual attn weights shape: {attn_weights.shape}")
        # print(f"[DEBUG] Key states shape: {key_states.shape}")
