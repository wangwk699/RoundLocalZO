...
def smooth_ln_fcs_inplace(ln, fcs, scales,shifts):
    ...
    for fc in fcs:
        ...
        if hasattr(fc, 'bias') and fc.bias is not None:
            ...
            # ldx:add:
            # fc.register_buffer('bias',fc.orig_weight@shifts)
        fc.weight.mul_(scales.view(1,-1))
        # ldx:add:
        # fc.orig_weight.mul_(scales.view(1,-1))
...
def smooth_fc_fc_inplace(fc1, fc2, scales,shifts=None):
    ...
    # ldx:add:
    # fc1.orig_weight.div_(scales.view(-1,1))
    ...
    if hasattr(fc2, 'bias') and fc2.bias is not None:
        ...
        # ldx:add:
        # fc2.register_buffer('bias',fc2.orig_weight@shifts)
    fc2.weight.mul_(scales.view(1,-1))
    # ldx:add:
    # fc2.orig_weight.mul_(scales.view(1,-1))
...
def smooth_q_k_inplace(q_proj, k_proj, scales,):
    ...
    # ldx:add:
    # q_proj.orig_weight.div_(scales.view(-1,1))
    # q_proj.bias.div_(scales.view(-1))
    # k_proj.orig_weight.mul_(scales.view(-1,1))
    # k_proj.bias.mul_(scales.view(-1))